...
public class Service {
    ...
    public Service(String serviceCode, String serviceName, String description,
        ...
        try { setServiceCode(serviceCode); }
        catch (ServiceCodeException sce) {
            throw new ServiceException(sce.getMessage());
        }
    ...
    public void setServiceCode(String serviceCode) throws ServiceCodeException {
        ...
        // test 1 - is it the right length?
        if (serviceCode.length() != AppSettings.Service.SERVICE_CODE_LENGTH) {
            throw new ServiceCodeException(
                    String.format("The service must be a %d digit alphanumeric value",
                            AppSettings.Service.SERVICE_CODE_LENGTH));
        }

        // test 2 - is it the right pattern?
        Pattern p = Pattern.compile(AppSettings.Service.SERVICE_CODE_PATTERN);
        if (!p.matcher(serviceCode).find()) {
            throw new ServiceCodeException("Invalid Service Code Pattern");
        }

        // test 3 - is the service code unique
...
//</editor-fold>
    ...
    private void setDefaults() {
        ...
        this.serviceName = AppSettings.Service.DEFAULT_SERVICE_NAME;
